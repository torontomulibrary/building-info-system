...
import { fetchIMG, fetchJSON } from '../utils/fetch.js';
...
export const getMapData = (apiUrl, bldCode, floorNum) => async dispatch => {
  ...
  firstFloor = floorNum ?
    ...
    Object.values(firstBuilding.floors).filter(f => f.number === floorNum)[0] :
  ...
  await fetchIMG(firstFloor.floorplan).then(firstImg => {
    // Store the src for the loaded blob and set it as the active map.
    firstFloor.floorplan = firstImg;
